...
def detect_brightband_height(
    ...
    the 0 degC level is taken a few hundred metres above it (near the top of
    the bright band).

    Parameters
    ----------
    radarlist : list of xr.Dataset
        Per-sweep datasets for the volume.
    fields : dict
        Canonical->actual field name mapping (see resolve_fields).
    ...
    for radar in radarlist:
        z = np.asarray(radar.z.values, dtype="float64")
        zh = np.asarray(radar[fields["DBZH"]].values, dtype="float64")
        rho = np.asarray(radar[fields["RHOHV"]].values, dtype="float64")
        zdr = np.asarray(radar[fields["ZDR"]].values, dtype="float64")
        ...
        if good.any():
            heights.append(z[good])
            weights.append(0.97 - rho[good])
...
def get_volume_temperature_profile(
    ...
    z0c = detect_brightband_height(radarlist, fields)
    if z0c is not None:
...
def interp_temperature(
    geo_h_profile: np.ndarray, temp_profile: np.ndarray, gates_z: np.ndarray
) -> np.ndarray:
    """
    Interpolate a (heights_m, temperature_K) profile onto gate heights, returning
    degC (masked outside the profile range).
    """
    f_interp = interpolate.interp1d(
        geo_h_profile, temp_profile, bounds_error=False, fill_value=9999
    )
    return np.ma.masked_equal(f_interp(gates_z), 9999) - 273.15
